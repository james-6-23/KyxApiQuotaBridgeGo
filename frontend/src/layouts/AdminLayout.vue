  ...
  <a-layout class="min-h-screen">
    ...
    <a-layout :class="{ 'ml-0': isMobile, 'ml-60': !isMobile && !collapsed, 'ml-20': !isMobile && collapsed }">
      ...
      >
        ...
        <div class="flex items-center space-x-4">
          <!-- 系统健康状态 -->
          <a-tooltip :title="systemHealthText">
            <a-badge :status="systemHealthStatus" :text="!isSmallScreen ? systemHealthText : ''" />
          </a-tooltip>
          ...
          <a-dropdown placement="bottomRight">
            <div class="flex items-center space-x-2 cursor-pointer hover:bg-gray-100 px-3 py-2 rounded-lg transition-colors">
              <a-avatar :size="32" class="bg-red-500">
                <template #icon>
                  <CrownOutlined />
                </template>
              </a-avatar>
              <span v-if="!isSmallScreen" class="text-gray-700 font-medium">管理员</span>
      ...
      <a-layout-footer class="text-center text-gray-600">
        <div class="space-y-2">
          <div>KYX API Quota Bridge - 管理后台 © {{ currentYear }}</div>
...
// ==================== Computed ====================
const loading = computed(() => adminStore.loading || appStore.loading)
